...
public class RoomEventConsumer {
	...
	public void subscribe() {
		consumer.receiveAutoAck() // topics already set in ReceiverOptions
				...
				.doOnError(ex -> log.error("Kafka stream error: {}", ex.getMessage(), ex)).retry().subscribe();
	}
	...
	private Mono<Void> handleRecord(final ConsumerRecord<String, RoomEventEnvelope<?>> record) {
		try {
			...
			// 'data' is still a LinkedHashMap (generic erased) -> convert to your payload
			// type
			...
			log.error("Failed to process record key={} offset={}: {}", record.key(), record.offset(), e.getMessage(),
					e);
