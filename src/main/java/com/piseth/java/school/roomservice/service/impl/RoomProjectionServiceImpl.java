...
public class RoomProjectionServiceImpl implements RoomProjectionService {
	...
	public Mono<Void> apply(final RoomEventEnvelope<RoomFullPayload> env) {
		...
		return eventRepo
				.findByAggregateTypeAndAggregateId("Room", roomId).defaultIfEmpty(ProcessedEvent.builder()
						.aggregateType("Room").aggregateId(roomId).lastEventId(null).lastOccurredAt(null).build())
				.flatMap(pe -> {
	...
	private Mono<Void> upsertProjection(final RoomFullPayload payload, final RoomEventEnvelope<RoomFullPayload> env,
			final ProcessedEvent pe) {
		return roomRepo.findById(payload.getId()).defaultIfEmpty(mapper.toProjection(payload)).flatMap(existing -> {
	...
	private Mono<Void> softDelete(final String id, final RoomEventEnvelope<RoomFullPayload> env,
			final ProcessedEvent pe) {
		return roomRepo.findById(id).flatMap(doc -> {
			doc.setDeleted(true);
			doc.setLastEventAt(LocalDateTime.ofInstant(env.getOccurredAt(), java.time.ZoneOffset.UTC));
			return roomRepo.save(doc);
		}).switchIfEmpty(Mono.empty()).then(updateProcessed(env, pe));
	}

	private Mono<Void> updateProcessed(final RoomEventEnvelope<RoomFullPayload> env, final ProcessedEvent pe) {
